...
/* BUGGY 3-WAY SPLIT (skims C) — Cron-native fair distribution to three beneficiaries.
 *
 * Each cron fire distributes PER: SHA to PYA, SHB to PYB, the remainder PER-SHA-SHB to PYC.
 * The sum emitted is EXACTLY PER on every fire — the Hook cannot skim a cut or short a beneficiary.
 * Cron-fired (weak TSH, no rollback -> proof, not trust). A 3-way split; proves split-conservation generalizes to N.
 *
...
int64_t hook(uint32_t reserved)
{
    ...
    uint8_t pyc_key[3] = { 'P', 'Y', 'C' };
    uint8_t pyc[20];
    XAHC_HOOK_PARAM_REQUIRE(pyc, pyc_key, 20);
    ...
    uint8_t shb_key[3] = { 'S', 'H', 'B' };
    uint8_t shb_b[8];
    XAHC_HOOK_PARAM_REQUIRE(shb_b, shb_key, 8);
    uint64_t shb = be64(shb_b);
    ...
    /* bound EACH share <= per (< 2^62) FIRST, so sha+shb < 2^63 cannot u64-wrap before the <= check.
     * Without these a huge sha could wrap sha+shb to a small ab that passes, then emit the huge sha
     * -> over-distribute (the prover caught exactly this). */
    XAHC_REQUIRE(sha <= per, "share A exceeds the period total");
    XAHC_REQUIRE(shb <= per, "share B exceeds the period total");
    uint64_t ab = sha + shb;
    XAHC_REQUIRE(ab <= per, "shares exceed the period total");
    uint64_t c_amt = per - ab;
    ...
    XAHC_TRY(etxn_reserve(3));
    emit_one(pya, sha);
    emit_one(pyb, shb);
    /* BUG: drops PYC -> skim PER-SHA-SHB */
