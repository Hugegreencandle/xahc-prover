#include "xahc/xahc.h"
/* SC05 ADVERSARIAL twin — PARTIAL reserve: emits `amount` but records only amount/2.
 * Must -> COUNTEREXAMPLE (cover): spent' = spent + amount/2 < spent + amount (emitted). */
...
int64_t hook(uint32_t reserved_arg){
    ...
    uint64_t half=amount/2;                       /* BUG: records only half of what it emits */
    wr64(&sval[0],reserved+half);wr64(&sval[8],spent+half);XAHC_STATE_SET(skey,sval);
