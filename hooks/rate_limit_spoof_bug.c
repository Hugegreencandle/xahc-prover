...
int64_t hook(uint32_t r){
    ...
    XAHC_REQUIRE(now>=last+cd, "cooldown not elapsed");
    uint8_t tk[2]={'T','S'}, ts[8]; XAHC_HOOK_PARAM_REQUIRE(ts,tk,8);
    wr64(sval, be64(ts));                                 /* stamps an ATTACKER value (BUG) */
    XAHC_STATE_SET(skey,sval);
    XAHC_ACCEPT("spoofable stamp (BUG)");
    return 0;
}
