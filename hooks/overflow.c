...
/* CORRECT arithmetic — same as overflow_bug but with an overflow guard: total must not be
 * less than drops, which catches the uint64 wrap before the limit check. */
...
int64_t hook(uint32_t reserved)
{
    ...
    uint64_t total = drops + tip;
    XAHC_REQUIRE(total >= drops, "overflow");     /* wrap guard */
