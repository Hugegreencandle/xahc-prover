...
int64_t hook(uint32_t reserved)
{
    ...
    /* NEGATIVE CONTROL BUG: checks amount against PLM directly, FORGETTING to
     * subtract the prior `spent`. This lets cumulative spend blow past PLM. */
    XAHC_REQUIRE(amount <= plm, "over period budget (BUGGY: ignores prior spent)");
