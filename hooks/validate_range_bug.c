#include "xahc/xahc.h"
/* SC04 NEGATIVE CONTROL: checks presence + LOWER bound only, OMITS the upper bound ->
 * accepts VAL > HI (out of declared range) -> COUNTEREXAMPLE. */
...
int64_t hook(uint32_t r){
    ...
    uint64_t val=be64(vb), lo=be64(lb);
