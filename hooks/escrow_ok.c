...
/* PROVABLE HASHLOCK ESCROW (HTLC) — release ONLY to whoever knows the preimage of a committed hash.
 *
 ...
 * CLAIM-ONCE relies on Xahau's emit/accept ATOMICITY: emits flush ONLY when the hook accepts; the
 * post-emit `state_set(spent=1)` is XAHC_REQUIRE'd, so a state_set failure rolls back the WHOLE hook
 * (the staged release is discarded, tecHOOK_REJECTED) — emit and spent-flag commit together or not at all.
 *
...
int64_t hook(uint32_t reserved)
{
    ...
    if (!match)
        XAHC_ACCEPT("wrong preimage — no release");
